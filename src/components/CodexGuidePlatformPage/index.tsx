...
import {
  ...
  getCodexGuidePlatformSectionBySlug,
} from '@site/src/data/codexGuideTopic';
import {
  getCodexGuidePlatformPage,
  getCodexGuidePlatformSidebar,
  type CodexGuidePlatformPageData,
} from '@site/src/data/codexGuidePlatformPages';
import styles from '../CodexGuideGuidePage/styles.module.css';

function buildInitialExpandedState(page: CodexGuidePlatformPageData) {
  const currentSection = getCodexGuidePlatformSectionBySlug(page.slug);
  const initial: Record<string, boolean> = {};

  for (const section of getCodexGuidePlatformSidebar()) {
...
export default function CodexGuidePlatformPage({slug}: {slug: string}): ReactNode {
  const page = getCodexGuidePlatformPage(slug);
  ...
  return (
    <Layout title={`${page.title} | CodexGuide`} description={page.summary}>
      <div className={styles.page}>
        <CodexGuideTopicNav activeKey="platform" />

        <div className={`container ${styles.layout}`}>
          <aside className={styles.sidebar}>
            {getCodexGuidePlatformSidebar().map((section) => {
                  ...
                  {expanded ? (
                    <ul className={styles.sidebarLinks}>
                      {section.items.map((item) => {
                        const to = getCodexGuidePlatformRoute(item.slug);
          ...
          <main className={styles.main}>
            ...
            <div className={styles.footerNav}>
              {page.prevSlug ? (
                <Link className={styles.footerNavItem} to={getCodexGuidePlatformRoute(page.prevSlug)}>
                  <span>上一页</span>
                  <strong>{getCodexGuidePlatformPage(page.prevSlug)?.title}</strong>
                </Link>
              ) : (
                <span />
              )}
              ...
              {page.nextSlug ? (
                <Link className={styles.footerNavItem} to={getCodexGuidePlatformRoute(page.nextSlug)}>
                  <span>下一页</span>
                  <strong>{getCodexGuidePlatformPage(page.nextSlug)?.title}</strong>
                </Link>
              ) : null}
            </div>
          </main>
