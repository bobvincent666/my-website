...
import {
  ...
  getCodexGuidePracticeSectionBySlug,
} from '@site/src/data/codexGuideTopic';
import {
  getCodexGuidePracticePage,
  getCodexGuidePracticeSidebar,
  type CodexGuidePracticePageData,
} from '@site/src/data/codexGuidePracticePages';
import styles from '../CodexGuideGuidePage/styles.module.css';

function buildInitialExpandedState(page: CodexGuidePracticePageData) {
  const currentSection = getCodexGuidePracticeSectionBySlug(page.slug);
  const initial: Record<string, boolean> = {};

  for (const section of getCodexGuidePracticeSidebar()) {
...
export default function CodexGuidePracticePage({slug}: {slug: string}): ReactNode {
  const page = getCodexGuidePracticePage(slug);
  ...
  return (
    <Layout title={`${page.title} | CodexGuide`} description={page.summary}>
      <div className={styles.page}>
        <CodexGuideTopicNav activeKey="practice" />

        <div className={`container ${styles.layout}`}>
          <aside className={styles.sidebar}>
            {getCodexGuidePracticeSidebar().map((section) => {
                  ...
                  {expanded ? (
                    <ul className={styles.sidebarLinks}>
                      {section.items.map((item) => {
                        const to = getCodexGuidePracticeRoute(item.slug);
          ...
          <main className={styles.main}>
            ...
            <div className={styles.footerNav}>
              {page.prevSlug ? (
                <Link className={styles.footerNavItem} to={getCodexGuidePracticeRoute(page.prevSlug)}>
                  <span>上一页</span>
                  <strong>{getCodexGuidePracticePage(page.prevSlug)?.title}</strong>
                </Link>
              ) : (
                <span />
              )}
              ...
              {page.nextSlug ? (
                <Link className={styles.footerNavItem} to={getCodexGuidePracticeRoute(page.nextSlug)}>
                  <span>下一页</span>
                  <strong>{getCodexGuidePracticePage(page.nextSlug)?.title}</strong>
                </Link>
              ) : null}
            </div>
          </main>
