...
import {
  CODEX_GUIDE_ROUTE,
  getCodexGuideSidebarSectionBySlug,
  getCodexGuideGuideRoute,
} from '@site/src/data/codexGuideTopic';
import {
  getCodexGuideGuidePage,
  getCodexGuideGuideSidebar,
  type CodexGuideGuidePageData,
} from '@site/src/data/codexGuideGuidePages';
import styles from './styles.module.css';

function buildInitialExpandedState(page: CodexGuideGuidePageData) {
  const currentSection = getCodexGuideSidebarSectionBySlug(page.slug);
  const initial: Record<string, boolean> = {};

  for (const section of getCodexGuideGuideSidebar()) {
...
export default function CodexGuideGuidePage({slug}: {slug: string}): ReactNode {
  const page = getCodexGuideGuidePage(slug);
  ...
  return (
    <Layout title={`${page.title} | CodexGuide`} description={page.summary}>
      <div className={styles.page}>
        <CodexGuideTopicNav activeKey="guide" />

        <div className={`container ${styles.layout}`}>
          <aside className={styles.sidebar}>
            {getCodexGuideGuideSidebar().map((section) => {
                  ...
                  {expanded ? (
                    <ul className={styles.sidebarLinks}>
                      {section.items.map((item) => {
                        const to = getCodexGuideGuideRoute(item.slug);
          ...
          <main className={styles.main}>
            ...
            {page.surfaceMap ? (
              <section className={styles.entryMap}>
                <Heading as="h2">{page.surfaceMap.title}</Heading>
                <p>{page.surfaceMap.description}</p>
                <div className={styles.entryGrid}>
                  {page.surfaceMap.primaryCards.map((card) => (
                    <div key={card.title} className={styles.entryCard} data-tone={card.tone}>
                      <strong>{card.title}</strong>
                      <span>{card.description}</span>
                    </div>
                  ))}
                </div>
                <div className={styles.entryWideRow}>
                  {page.surfaceMap.secondaryCards.map((card) => (
                    <div key={card.title} className={styles.entryWide}>
                      <strong>{card.title}</strong>
                      <span>{card.description}</span>
                    </div>
                  ))}
            ...
            <div className={styles.footerNav}>
              {page.prevSlug ? (
                <Link className={styles.footerNavItem} to={getCodexGuideGuideRoute(page.prevSlug)}>
                  <span>上一页</span>
                  <strong>{getCodexGuideGuidePage(page.prevSlug)?.title}</strong>
                </Link>
              ) : (
                <span />
              )}
              ...
              {page.nextSlug ? (
                <Link className={styles.footerNavItem} to={getCodexGuideGuideRoute(page.nextSlug)}>
                  <span>下一页</span>
                  <strong>{getCodexGuideGuidePage(page.nextSlug)?.title}</strong>
                </Link>
              ) : null}
            </div>
          </main>
